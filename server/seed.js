...
const CASES = [
  {
    ...
    "category": "写作", "difficulty": "入门",
    "timeCost": "每天30分钟", "income": "500-2000元/月",
    "problem": "很多小商家和店主想在小红书上推广，但自己不会写文案，请专业写手又太贵。",
    ...
    "steps": [
      "注册 Claude 账号（claude.ai），有免费额度先用着",
      "去小红书搜你目标领域（美妆/美食/家居）的爆款笔记，收藏5篇作为参考",
      "让 Claude 分析这些爆款的共同点：'分析这5篇爆款文案的风格、结构和用词特点'",
      "用提示词生成初稿：'帮我写一篇[产品名]的小红书种草文案，目标用户是[谁]，风格参考[参考文案]，包含：痛点引入+产品亮点+使用感受+推荐'",
      "手动改一下，加入「我用了之后…」「朋友都问我…」这种个人体验感",
      "去闲鱼挂服务：'AI文案代写，一篇XX元，包修改到满意'"
    ],
    "tools": ["Claude", "小红书", "闲鱼"],
  ...
  },
  {
    ...
    "steps": [
      "选一个 AI 绘画工具：国内的用通义万相/文心一格（免费），国外的用 Midjourney（付费但效果更好）",
      "学写提示词：风格+主体+氛围+比例。例如：'治愈系动漫风格，一只柴犬坐在咖啡杯旁，日落光线，柔和色彩，9:16竖版手机壁纸'",
      "批量生成50张不同风格的，挑最好看的20张上传闲鱼",
      "闲鱼开店：标题写'原创手机壁纸 定制头像 5元起 不满意重做'",
      "有人下单后，按他要求的风格现做，5分钟出图，发百度网盘链接交付"
    ],
  ...
  },
  {
    ...
    "steps": [
      "选定一个垂直领域：美食、旅游、知识科普、好物推荐…选你最熟悉或感兴趣的",
      "在该领域找10个爆款视频，让 Claude 分析：'分析这10个爆款脚本的结构，总结出3种万能公式'",
      "用 Claude 批量生成脚本：'用[公式一]写一个[主题]的60秒短视频脚本，开头2秒要有钩子，中间有反转/干货，结尾引导关注'",
      "在猪八戒/闲鱼/接单群挂服务：'短视频脚本代写，一条XX元，可试写一条免费'",
      "签3-5个稳定客户，每月固定供稿"
    ],
  ...
  },
  {
    ...
    "steps": [
      "在闲鱼挂服务：'Excel数据处理，10元一次，量大优惠。支持：数据整理/分类汇总/生成图表'",
      "客户发来数据（比如一堆销售记录或报名表），你直接发给 Claude",
      "提示词：'帮我把这份数据整理成表格，按日期排序，统计每个产品的销量和金额，生成一个柱状图'",
      "把 AI 整理好的结果保存，检查一下数据对不对",
      "发给客户，收钱"
    ],
  ...
  },
  {
    ...
    "steps": [
      "注册 Gamma.ai 或美册AI（国内），这些工具输入大纲就能自动生成PPT",
      "让 Claude 生成内容：'帮我写一份[主题]的PPT大纲，包含封面+目录+6页正文+结束页，每页配图建议'",
      "把大纲导入 Gamma，选好模板，自动生成",
      "在淘宝/闲鱼开店：'PPT代做，一页10元起，加急另算'",
      "高质量路演PPT可以报价500-2000元一单"
    ],
  ...
  },
  {
    ...
    "steps": [
      "让 AI 翻译：'请把以下内容翻译成英文，保持专业语气。注意：[具体注意事项]'",
      "逐段读一遍，重点检查：专业术语是否准确、文化表达是否地道、语气是否合适",
      "修正后放到 Grammarly 里跑一遍语法检查",
      "在 Upwork/fiverr/淘宝 挂服务：'专业翻译+人工校对，中英互译'",
      "积累案例后专注一个领域（比如跨境电商翻译），报价可以翻倍"
    ],
...
async function seed() {
  ...
  if (!existing) {
    await Admin.create({ email, password: 'admin123', name: '管理员' });
  ...
  if (count === 0) {
    ...
    console.log(`已导入 ${CASES.length} 个案例`);
  } else {
    console.log(`数据库已有 ${count} 个案例，跳过导入`);
  }
